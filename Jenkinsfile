pipeline {
  ...
  stages {
        ...
        withCredentials([
          ...
          dir('frontend') {
            bat '''
              ...
              rem === ใช้ TAG ให้ตรงกับขั้น Build/Push = v.1.0 ===
              set "TAG=v.1.0"
    ...
    stage('Build Backend') {
      steps {
        dir('backend') {
          bat 'docker build . -t mmmmnl/lobotomy:v.1.0'
        }
      }
    }

    stage('Login & Push Backend image') {
      steps {
        withCredentials([usernamePassword(credentialsId: 'docker-hub-creds',
                                          usernameVariable: 'DOCKERHUB_USERNAME',
                                          passwordVariable: 'DOCKERHUB_PASSWORD')]) {
          bat '''
            echo %DOCKERHUB_PASSWORD% | docker login -u %DOCKERHUB_USERNAME% --password-stdin
            docker push mmmmnl/lobotomy:v.1.0
          '''
        }
      }
    }
    ...
    stage('Deploy Backend to K8s') {
      steps {
        withCredentials([
          file(credentialsId: 'kubeconfig-prod',   variable: 'KUBECONFIG_FILE'),
          file(credentialsId: 'backend-env-file',  variable: 'ENV_FILE_BACK')
        ]) {
          dir('backend') {
            bat '''
              setlocal EnableDelayedExpansion
  ...
  post {
    success { echo '✅ One-pipeline: Frontend + Backend สำเร็จครบ' }
    failure { echo '❌ One-pipeline ล้มเหลว – ตรวจ log ของสเตจที่พัง' }
  }
}
