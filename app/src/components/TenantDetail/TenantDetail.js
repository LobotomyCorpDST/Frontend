...
const RoomCard = ({ room }) => {
    ...
    return (
        <Card
            ...
            data-cy={`tenant-detail-room-card-${room.number}`}
        >
            <CardContent
                sx={{
                    display: 'flex',
            ...
            >
                <Typography
                    ...
                    component="div"
                    sx={{ fontWeight: 'bold' }}
                    data-cy={`tenant-detail-room-card-number-${room.number}`}
                >
                    {room.number}
                </Typography>
                <Box>
                    <Typography
                        ...
                        data-cy={`tenant-detail-room-card-status-${room.number}`}
                    >
...
const TenantDetail = () => {
    ...
    if (loading)
        return (
            <Box
                ...
                data-cy="tenant-detail-loading-state"
            >
                <CircularProgress />
            </Box>
        );
    if (error)
        return (
            <Box sx={{ p: 3 }} data-cy="tenant-detail-error-state">
                <Alert severity="error" data-cy="tenant-detail-error-message">{error}</Alert>
            </Box>
        );
    if (!tenant)
        return (
            <Box sx={{ p: 3 }} data-cy="tenant-detail-not-found-state">
                <Alert severity="warning">Tenant not found.</Alert>
            </Box>
        );

    return (
        <Box className="detail-page-container" data-cy="tenant-detail-page-container">
            {/* Left Panel: Tenant & Lease Info */}
            <Paper elevation={3} className="left-panel" data-cy="tenant-detail-left-panel">
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <IconButton
                        onClick={() => navigate(-1)}
                        data-cy="tenant-detail-back-button"
                    >
                        <ArrowBackIcon />
                    </IconButton>
                    <Typography
                        ...
                        component="h1"
                        sx={{ ml: 1 }}
                        data-cy="tenant-detail-tenant-name-title"
                    >
                        {tenant.name}
                    </Typography>
                </Box>
                ...
                <Box className="scrollable-content">
                    ...
                    <Paper
                        variant="outlined"
                        sx={{ p: 3, mb: 2 }}
                        data-cy="tenant-detail-info-card"
                    >
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={12} sm={6}>
                                ...
                                <Typography data-cy="tenant-detail-info-name">{tenant.name}</Typography>
                            </Grid>
                            <Grid item xs={12} sm={6}>
                                <Typography variant="caption" display="block">
                                    ช่องทางติดต่อ
                                </Typography>
                                <Typography data-cy="tenant-detail-info-line">
                                    <strong>LINE:</strong> {tenant.lineId || '-'}
                                </Typography>
                                <Typography data-cy="tenant-detail-info-phone">
                    ...
                    {leases.length > 0 ? (
                        leases.map((lease, idx) => (
                            <Paper
                                ...
                                variant="outlined"
                                sx={{ p: 3, mb: 2 }}
                                data-cy={`tenant-detail-lease-card-${lease.id}`}
                            >
                                <Typography
                                    variant="h6"
                                    gutterBottom
                                    data-cy={`tenant-detail-lease-card-title-${lease.id}`}
                                >
                                ...
                                <Grid container spacing={2}>
                                    <Grid item xs={12} sm={6}>
                                        <Typography data-cy={`tenant-detail-lease-card-check-in-${lease.id}`}>
                                            <strong>วันที่เข้า:</strong> {fmt(lease?.startDate)}
                                        </Typography>
                                        <Typography data-cy={`tenant-detail-lease-card-check-out-${lease.id}`}>
                                            <strong>วันที่ออก:</strong> {fmt(lease?.endDate)}
                                        </Typography>
                                    </Grid>
                                    <Grid item xs={12} sm={6}>
                                        <Typography data-cy={`tenant-detail-lease-card-start-date-${lease.id}`}>
                                            <strong>สัญญาเริ่ม:</strong> {fmt(lease?.startDate)}
                                        </Typography>
                                        <Typography data-cy={`tenant-detail-lease-card-end-date-${lease.id}`}>
                        ...
                        <Paper
                            variant="outlined"
                            sx={{ p: 3, mb: 2 }}
                            data-cy="tenant-detail-no-lease-message"
                        >
                ...
                <Box className="action-footer">
                    <Button
                        ...
                        data-cy="tenant-detail-edit-button"
                    >
                        แก้ไขข้อมูล
                    </Button>
                </Box>
            ...
            <Paper elevation={3} className="right-panel" data-cy="tenant-detail-right-panel">
                <Typography variant="h6" color="primary" className="section-header">
                    ห้องที่เช่า
                </Typography>
                <Box sx={{ p: 2, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    {leases.length > 0 ? (
                        ...
                        <Typography data-cy="tenant-detail-no-rooms-message">ไม่มีห้องเช่า</Typography>
                    )}
                </Box>
            </Paper>
            ...
            {isEditModalOpen && (
                <EditTenantModal
                    ...
                    tenantId={tenant.id}
                    onUpdated={loadData}
                    data-cy="tenant-detail-edit-modal"
                />
            )}
        </Box>
    );
