    ...
    if (loading) {
        return (
            <Box
                ...
                data-cy="supply-inventory-loading-state"
            >
        ...
        <Box sx={{ p: 3 }} {...props} data-cy="supply-inventory-page">
            ...
            {error && (
                <Alert
                    ...
                    onClose={() => setError('')}
                    sx={{ mb: 2 }}
                    data-cy="supply-inventory-error-alert"
                >
                    {error}
                </Alert>
            )}
            ...
            <TableContainer
                ...
                data-cy="supply-inventory-table-container"
            >
                <Table sx={{ minWidth: 650 }} aria-label="supply inventory table">
                    <StandardTableHeader
                        ...
                        data-cy="supply-inventory-table-header"
                    />
                    <TableBody data-cy="supply-inventory-table-body">
                        {paginatedSupplies.length > 0 ? (
                            paginatedSupplies.map((supply) => (
                                <TableRow
                                    ...
                                    data-cy={`supply-inventory-row-${supply.id}`}
                                >
                                    <TableCell data-cy={`supply-inventory-cell-name-${supply.id}`}>
                                        {editingId === supply.id ? (
                                            <TextField
                                                size="small"
                                                value={editName}
                                                ...
                                                fullWidth
                                                data-cy={`supply-inventory-edit-name-input-${supply.id}`}
                                            />
                                        ) : (
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                ...
                                                {isLowStock(supply.supplyAmount) && (
                                                    <Chip
                                                        ...
                                                        color="error"
                                                        size="small"
                                                        data-cy={`supply-inventory-low-stock-chip-${supply.id}`}
                                                    />
                                                )}
                                            </Box>
                                        )}
                                    </TableCell>
                                    <TableCell align="center" data-cy={`supply-inventory-cell-amount-${supply.id}`}>
                                        {editingId === supply.id ? (
                                            <TextField
                                                size="small"
                                                type="number"
                                                ...
                                                sx={{ width: 100 }}
                                                data-cy={`supply-inventory-edit-amount-input-${supply.id}`}
                                            />
                                        ) : (
                                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                                                <IconButton
                                                    ...
                                                    data-cy={`supply-inventory-decrement-button-${supply.id}`}
                                                >
                                                    <RemoveIcon />
                                                </IconButton>
                                                <Typography
                                                    sx={{ minWidth: 40, textAlign: 'center' }}
                                                    data-cy={`supply-inventory-amount-text-${supply.id}`}
                                                >
                                                    {supply.supplyAmount}
                                                </Typography>
                                                <IconButton
                                                    size="small"
                                                    onClick={() => handleIncrement(supply.id)}
                                                    data-cy={`supply-inventory-increment-button-${supply.id}`}
                                                >
                                                    <AddIcon />
                                                </IconButton>
                                            </Box>
                                        ...
                                        {editingId === supply.id ? (
                                            <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                                                <Button
                                                    ...
                                                    variant="contained"
                                                    onClick={saveEditing}
                                                    data-cy={`supply-inventory-save-button-${supply.id}`}
                                                >
                                                    บันทึก
                                                </Button>
                                                <Button
                                                    ...
                                                    data-cy={`supply-inventory-cancel-button-${supply.id}`}
                                                >
                                                    ยกเลิก
                                                </Button>
                                            </Box>
                                            ...
                                            <Box sx={{ display: 'flex', gap: 1, justifyContent: 'center' }}>
                                                ...
                                                <IconButton
                                                    ...
                                                    data-cy={`supply-inventory-delete-button-${supply.id}`}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </Box>
                            ...
                            <TableRow data-cy="supply-inventory-no-data-row">
                ...
                {filteredSupplies.length > 0 && (
                    <StandardPagination
                        ...
                        data-cy="supply-inventory-pagination"
                    />
                )}
            </TableContainer>
            ...
            <AddSupplyModal
                ...
                data-cy="supply-inventory-add-modal"
            />
        </Box>
    );
};
