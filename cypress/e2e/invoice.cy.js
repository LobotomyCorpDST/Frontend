describe('Invoice list page test', () => {
    ...
    //     cy.get('[data-cy="invoice-history-table"]').find('[data-cy^="invoice-history-row-room-"]').first().should('contain', '201');
    //     cy.get('[data-cy="invoice-history-table"]').find('[data-cy^="invoice-history-row-room-"]').first().click();
    // })
    //
    // it('smart search by invoice id', () =>{
    //     cy.get('[data-cy="smart-search-input-field"]').type('184{enter}');
    //     cy.get('[data-cy="invoice-history-table"]').find('[data-cy^="invoice-history-row-id-"]').first().should('contain', '184');
    // })
    //
    // it('smart search by room number', () =>{
    //     cy.get('[data-cy="smart-search-input-field"]').type('4004{enter}');
    //     cy.get('[data-cy="invoice-history-table"]').find('[data-cy^="invoice-history-row-total-"]').first().should('contain', '4,004');
    // })
